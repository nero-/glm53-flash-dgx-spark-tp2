...
both the NVFP4-Spark and the non-spark NVFP4 quant) across DGX Spark
TP2 pairs (Grace + GB10, SM 12.1, 2×2 nodes: cluster 1 = gx10-r0/r1 daily
driver, cluster 2 = gx10-r2/r3 running the same image + profiles), using the
**Jovian Judgement** arm64 vLLM stack from
...
| Kernels | B12X master @ `3eb57add` (GB10 `--load-format b12x` O_DIRECT loader; the matched cut with the vLLM pin) + FlashInfer + patched NCCL 2.30.4 |
| Image | `local/vllm:glm53-flash-nvfp4-head0906-managed`, built with the arm64/SM121 retarget of `blackwell-llm-docker` (CUDA 13.2 / torch 2.13.0+cu132); the superseded lines (`devspark`, `devspark2`, `devc646`) were deleted from all four nodes' image stores on 2026-09-07 |
...
| Serving profiles | `mtp3-spark` (MTP3 + marlin, 10.5 GiB KV) · `mtp3-nvfp4` (MTP3 + humming, 6.5 GiB) · `df-spark` (DFlash2@7, 12.5 GiB) · `df-nvfp4` (DFlash2@7 + video, 4.5 GiB) — identical on both clusters |
| Fabric | DGX Spark pairs, NVIDIA-Sync-managed ConnectX-7 RoCEv2 (cluster 1: one <c1-fabric-subnet> rail on the f1 ports; cluster 2: same setup, its own <c2-fabric-subnet> rail) |
...
- [`env/`](env/) — the per-rank env templates: the four cluster-1 profiles
  (`rank-{0,1}-…`) plus the same four for cluster 2 (`rank-{2,3}-…`); the
  retired devc646-era `rank-{2,3}-{mtp3,dflash2}.env` remain as history only
